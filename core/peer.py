...
import hashlib
import asyncio
from core.message import MessageType, read_message, write_message
...
class DownloadManager:
    ...
    async def _update_peer_rtt(self, addresses):
        # Test multiple peer's rtt, must be registered
        # read_coro -> address
        read_tasks = set()
    ...
    async def _request_chunkinfo(self):
        ...
        assert MessageType(message['type']) == MessageType.REPLY_FILES_LOCATION
    ...
    async def _send_request_chunk(self, chunknum):
        ...
        try:
            await write_message(self._peers[fastest_peer][1], {
                'type': MessageType.PEER_REQUESTS_CHUNK,
                'filename': self._filename,
                'chunknum': chunknum
            })
        ...
        while len(self._file_chunk_info) != 0:
            ...
            for finished_task in done:
                ...
                try:
                    ...
                    try:
                        await write_message(self._tracker_writer, {
                            'type': MessageType.REQUEST_CHUNKS_REGISTER,
                            'filename': self._filename,
                            'chunknum': number
                        })
                    except (ConnectionError, RuntimeError):
...
class Peer(MessageServer):
    ...
    async def connect(self, tracker_address, loop=None):
        ...
        try:
            ...
            await write_message(self._tracker_writer, {
                'type': MessageType.REQUEST_REGISTERY,
                'address': self._server_address
            })
            message = await read_message(self._tracker_reader)
            assert MessageType(message['type']) == MessageType.REPLY_REGISTERY
    ...
    async def publish(self, local_file, remote_name=None):
        ...
        try:
            ...
            await write_message(self._tracker_writer, {
                'type': MessageType.REQUEST_TO_PUBLISH,
    ...
    async def list_file(self):
        ...
        try:
            await write_message(self._tracker_writer, {
                'type': MessageType.REQUEST_FILES_LIST,
            })
            message = await read_message(self._tracker_reader)
            assert MessageType(message['type']) == MessageType.REPLY_FILES_LIST
    ...
    async def download(self, file, destination, reporthook=None):
        ...
        # update chunk info every UPDATE_FREQUENCY chunks
    ...
    async def _process_connection(self, reader, writer):
        ...
        while not reader.at_eof():
            ...
            try:
                ...
                if message_type == MessageType.PEER_REQUESTS_CHUNK:
                    assert message['filename'] in self._file_map, 'File {} requested does not exist'.format(
                        message['filename'])
                    ...
                    await write_message(writer, {
                        'type': MessageType.PEER_REP_CHUNK,
